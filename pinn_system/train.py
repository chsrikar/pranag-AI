...
def main():
    ...
    parser.add_argument('--data',           type=str,   default='data/dataset.parquet')
    parser.add_argument('--lr',             type=float, default=0.001)
    parser.add_argument('--recompile_freq', type=int,   default=200)
    args = parser.parse_args()

    # 1. Load data - NO DUMMY DATA ALLOWED
    try:
        ...
        # For heat model, we need only (x, time) as input - 2D
        # The model expects: [x, time] not [time, x, temperature]
        if args.model == "heat":
            # Reorder to [x, time] and drop temperature
            X = X[:, [1, 0]]  # Swap columns: [time, x, temp] -> [x, time]
            logger.info(f'✅ Adjusted for heat model: X={X.shape} (x, time)')
        ...
        raise RuntimeError(f"Data loading failed. Please ensure {args.data} exists and is valid.") from e
